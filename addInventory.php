...
//!*    
        ...
        slog(0);
         $stmt= $db->prepare('CALL find_mfr(?)');
            ...
            $stmt->bind_param("is",$mfrID, $_POST['chemical']);
            if($stmt->execute())
            {
            ...
            if($mfrID==null){
                ...
                $stmt= $db->prepare("CALL Add_New_Inventory($chemID, ?, ?, ?, ?, ?);");
                $stmt->bind_param("ssiis",$_POST['room'], $_POST['location'],$_POST['quantity'], $_POST['unitSize'],$_POST['unit']);
                if(!($stmt->execute()))
                    slog($db->error);
             }
            /*$mfrID = null;
...
<body>
	...
	<div id="wrapper">
		<section id="main">
			<form class="inputField" id="addInv" action="addNewInventory.php" method="post"><fieldset>
				<p>
